...
const sexe = ["M", "F"]
export default function ModiffPersonnel({
  open,
  x,
  data_a_modifier,
  dcf_id,
  ...
  const CommuneSud = [];
  const FokotanySud = [];
  const [mydata, setmydata] = React.useState(data_a_modifier);
  ...
  const handleSubmit = async (e) => {
    ...
    if (mydata) {
      try {
        await axios.post(`http://localhost:8080/api/modifpersonnel/${id}`, mydata);
      } catch (error) {
        console.log(error);
      }
    }
        ...
        <ModalDialog>
          ...
          <form onSubmit={handleSubmit}>
            <Stack spacing={2} direction="column">
              ...
              <Stack direction="row" spacing={3}>
                ...
                <FormControl>
                  ...
                  <Autocomplete
                    options={district}
                    // value={diCoFo && diCoFo.district}
                    value={dcf_id !== undefined && dcf_id.district}
                    sx={{ width: 250 }}
            ...
            >
              ...
              <Button variant="outlined" color="danger" onClick={x}>
                Annuler
              </Button>
            </Stack>
          </form>
